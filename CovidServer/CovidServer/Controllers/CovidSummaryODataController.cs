using CovidServer.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.OData.Routing.Controllers;
using Microsoft.AspNetCore.OData.Query;
using Microsoft.EntityFrameworkCore;
using System.Linq;

namespace CovidServer.Controllers;

[Route("odata/CovidSummaryOData")]
public class CovidSummaryODataController : ODataController
{
    private readonly Prn3CovidContext _context;
    public CovidSummaryODataController(Prn3CovidContext context)
    {
        _context = context;
    }

    [EnableQuery]
    [HttpGet]
    public IQueryable<object> Get([FromQuery] string? type)
    {
        var query = _context.DailyMetrics.Include(dm => dm.Location).AsQueryable();
        
        if (type == "DailyIncrease")
        {
            // Lấy dữ liệu của 2 ngày gần nhất để tính daily increase
            var latestDate = query.Max(dm => dm.Date);
            var previousDate = latestDate.AddDays(-1);
            
            var currentDayData = query.Where(dm => dm.Date.Date == latestDate.Date);
            var previousDayData = query.Where(dm => dm.Date.Date == previousDate.Date);
            
            var result = from current in currentDayData
                        from previous in previousDayData.Where(p => p.LocationId == current.LocationId).DefaultIfEmpty()
                        group new { current, previous } by current.Location.CountryRegion into g
                        select new {
                            Country = g.Key,
                            Value = g.Sum(x => x.current.Confirmed ?? 0) - g.Sum(x => x.previous != null ? x.previous.Confirmed ?? 0 : 0),
                            Lat = g.Select(x => x.current.Location.Lat).FirstOrDefault(),
                            Long = g.Select(x => x.current.Location.Long).FirstOrDefault()
                        };
            
            return result.AsQueryable();
        }
        else
        {
            var grouped = query.GroupBy(dm => dm.Location.CountryRegion);
            return grouped.Select(g => new {
                Country = g.Key,
                Value = type == "Confirmed" ? g.Sum(x => x.Confirmed ?? 0)
                    : type == "Active" ? g.Sum(x => x.Active ?? 0)
                    : type == "Recovered" ? g.Sum(x => x.Recovered ?? 0)
                    : type == "Deaths" ? g.Sum(x => x.Deaths ?? 0)
                    : 0,
                Lat = g.Select(x => x.Location.Lat).FirstOrDefault(),
                Long = g.Select(x => x.Location.Long).FirstOrDefault()
            });
        }
    }
}
